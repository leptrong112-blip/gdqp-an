...
import { drawSkeleton } from '../rendering/skeletonRenderer';
...
export function usePoseSession() {
  ...
  const stop = useCallback(() => {
    release();
    if (mounted.current) { setStage('idle'); setSnapshot(null); setResult(null); setError(''); }
  }, [release]);

  const fail = useCallback((message: string) => {
    release();
    if (mounted.current) { setStage('error'); setError(message); setSnapshot(null); setResult(null); }
  }, [release]);
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => drawSkeleton(canvas, latest.current?.frame ?? null)); observer.observe(canvas);
    return () => observer.disconnect();
  }, []);

  const start = async () => {
    release(); setResult(null); setSnapshot(null); setError(''); setReduced(false);
    ...
    try {
      ...
      let budget = new AdaptiveBudget(), lastUi = 0, lastStage: PoseStage = 'quality-check', resolutionReduced = false;
      const engine = await createPoseRuntime(controller.signal, event => {
        ...
        if (event.type === 'score') {
          ...
          if (event.result.status === 'scored') {
            ...
            // Hiển thị trạng thái hoàn thành ngắn (1.2s)
            setStage('completed');
            setTimeout(() => {
        ...
        if (event.type === 'analysis') {
          ...
          if (!resolutionReduced && budget.width < 640) { resolutionReduced = true; void camera.reduceResolution(budget.width, budget.height); setReduced(true); }
          if (canvasRef.current) drawSkeleton(canvasRef.current, event.snapshot.frame);
      ...
      budget = new AdaptiveBudget(engine.fallback); setMode(engine.mode); setReduced(engine.fallback); setStage('quality-check');
  ...
  const retry = useCallback(() => {
    ...
    if (video && engine && !abort.current?.signal.aborted && startLoopRef.current) {
      setStage('calibrating');
      engine.command('startCalibration');
      startLoopRef.current();
    } else {
      void start();
    }
  ...
  const changeMovement = useCallback((id: MovementId) => {
    ...
    setScoreComparison(null);
    runtime.current?.command(getCommandForMovement(id));
  }, []);
