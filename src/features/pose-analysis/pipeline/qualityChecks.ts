...
import { measurements } from './normalization';
...
export class QualityChecker {
  ...
  check(frame: CanonicalPoseFrame, lighting: LightingMetrics, calibration?: CalibrationProfile, options?: { allowTurn?: boolean }): QualityReport {
    ...
    const framing = headroom && valid.length >= minFramingValid && valid.every(v => v.image.x / frame.aspectRatio > C.frameMargin && v.image.x / frame.aspectRatio < 1 - C.frameMargin && v.image.y > C.frameMargin && v.image.y < 1 - C.frameMargin);
    const footVisible = allowTurn || (['left', 'right'] as const).every(side => { const heel = p[`${side}Heel`], toe = p[`${side}FootIndex`]; return heel && toe && distance(flat(heel.image), flat(toe.image)) > length * 0.025; });
    const checks: QualityCheck[] = [
      ...
      { id: 'reliability', label: 'Khớp rõ ràng', passed: allowTurn ? (rollingCoverage >= 0.75 && rollingConfidence >= 0.6) : (rollingCoverage >= C.reliabilityCoverage && rollingConfidence >= C.reliabilityMean && coverage === 1 && confidence >= C.reliabilityMean), message: 'Giữ các khớp không bị che khuất và hướng người về camera.' },
